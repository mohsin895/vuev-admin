              ...
              <tr :class="(++dataIndex%2==0) ? 'table-primary':''">
                ...
                <td>
                  <slot v-if="$store.state.userInfo.permissions.includes(30)">
                    <a href="#" @click="prepareCaseToHighCourt(dataInfo)" v-b-modal.data-view>
        ...
        <b-row class="text-center mt-5">
          <b-col cols="12" class="d-flex flex-row-reverse">
             <slot v-if="$store.state.userInfo.permissions.includes(30)">
                ...
                <button type="button" class="btn btn-success" @click="sendToHighCourt()">সেভ করুন</button>
              </slot>
          </b-col>
          <b-col cols="6">
            <!--  -->
...
export default {
  ...
  methods:{
    ...
    prepareCaseToHighCourt(dataInfo){
      ...
      this.listShow=true;
      if(this.caseInfos.higher_court_info!=null)
        this.date=dataInfo.higher_court_info.created_at;
    },
    async getDataList(){
        let token=window.localStorage.getItem("token");
        let config = {
        ...
        let url="staff/get/case/list/for/higher/court?page="+this.pageNo+"&numOfData="+this.numOfData+"&status="+this.status+"&searchKey="+this.searchKey+"&unionId="+this.union+"&upazilaId="+this.upazila;
